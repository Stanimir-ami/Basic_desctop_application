...
def render_main_view():
    clear_view()
    Button(tk, text='List all tasks', bg='skyblue', fg='blue', command=render_list_tasks_view).grid(column=0, row=0, padx=45, pady=45)
    Button(tk, text='Create task', bg='green', fg='white', command=render_create_view).grid(column=1, row=0, padx=45, pady=45)
    Button(tk, text='Edit task', bg='yellow', fg='black', command=edit_task_view).grid(column=2, row=0, padx=20, pady=20)


def edit_description_task(task, new_description):
    task_as_obj = eval(task)  # eval прави стринга на обект(речник)
    if task:
        with open('db.txt', 'r+') as file:  # r+ тоест чете и пише
            all_tasks = json.loads(file.read())
            for index in range(len(all_tasks)):
                if all_tasks[index]['description'] == task_as_obj['description']:
                    break
            all_tasks[index]['description'] = new_description
            file.truncate(0)  # изтриваме всичко
            file.seek(0)  # започваме от началото
            json.dump(all_tasks, file)
            answer = messagebox.showinfo(title='Success operation', message='Task edited!')


def edit_task_view(): # няма функционалност
    clear_view()
    try:
    ...
    dropdown.grid(column=2, row=1, padx=20, pady=20)
    Label(tk, text='Select').grid(column=1, row=1, padx=20, pady=20) # добави команда
    edit_description = Entry(tk)
    edit_description.grid(column=2, row=2)
    Label(tk, text="Edit description:").grid(column=1, row=2, padx=20, pady=20)
    Button(tk, text='Edit', command=lambda: edit_description_task(selected_task.get(), edit_description.get())).grid(column=2, row=4)
    Button(tk, text='Back', bg='green', fg='white', command=render_main_view).grid(column=2, row=9)
...
def delete_task(task):
    answer = messagebox.askquestion('Delete task', 'Are you sure?')
    if answer == 'no':
        return
    task_as_obj = eval(task) # eval прави стринга на обект(речник)
    if task:
        with open('db.txt', 'r+') as file: # r+ тоест чете и пише
            ...
            file.truncate(0) # изтриваме всичко
            file.seek(0) # започваме от началото
            json.dump(all_tasks, file) # записваме всичко останало след триенето
...
def render_create_view():
    ...
    Button(tk, text='Create task', bg='green', fg='white', command=lambda:create_task(task_name.get(), task_description.get('1.0', END), task_priority.get(), task_completed.get())).grid(column=0, row=4)
    Button(tk, text='Cancel', bg='black', fg='white',
